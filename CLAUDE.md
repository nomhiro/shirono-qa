...
### 1.1 目的
転職後の個人的なQA受付サイトを構築し、元職場の同僚からの技術的な質問に対応する。

### 1.2 スコープ
- IT、AI、Azureに関する技術的な質問の受付と回答
...
- **詳細**:
  - 管理者がグループを定義・管理
  - ユーザーをグループに割り当て
  - グループ内ユーザーは同グループの質問を参照可能
  - 管理者は全質問を参照可能

#### 3.1.3 ユーザー権限
| 権限レベル   | 機能                                         |
| ------------ | -------------------------------------------- |
| 一般ユーザー | 質問投稿、同グループ内質問参照、コメント投稿 |
...
- **ステータス一覧**:
  - 未回答：投稿直後の初期状態
  - 回答済み：管理者が回答を投稿した状態
  - 解決済み：質問者が解決済みとマークした状態
  - 却下：管理者が質問を却下した状態
  - クローズ：運用終了等で質問を終了した状態
...
#### 3.5.1 自動タグ付け
- **概要**: Azure OpenAI GPT-4.1による質問の自動カテゴリ分類
- **タグカテゴリ**:
...
#### 6.2.3 画面モード遷移
- **一覧画面から質問詳細**: 質問タイトルクリックで表示モードで開く
- **新規投稿**: 「新規質問」ボタンで投稿モードで開く
- **編集モード**: 質問詳細画面の「編集」ボタンで編集モードに切り替え
...
2. アプリパスワードを生成：
   ...
   - アプリを選択: 「その他」で「QAサイト」と入力
3. 生成された16桁のパスワードをSMTP_PASSWORDに設定
4. 送信制限: 1日500通（QAサイトでは十分）
